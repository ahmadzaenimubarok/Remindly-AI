...
const TIER_CONFIG = {
  hot: {
    label: "Hot",
    desc: "Strong purchase intent signal — prioritize follow-up",
  ...
  warm: {
    label: "Warm",
    desc: "Interested but undecided — keep monitoring",
  ...
  cold: {
    label: "Cold",
    desc: "No interest signal yet — let AI handle",
...
const REASON_LABEL: Record<string, string> = {
  "niat_beli:positive": "Expressed purchase intent positively",
  "niat_beli:neutral": "Expressed purchase intent",
  "niat_beli:negative": "Expressed purchase intent, but hesitant",
  "niat_beli:unknown": "Expressed purchase intent",
  spam_only: "Only sent spam",
  no_interactions: "No interactions yet",
  single_interaction: "Only 1 interaction",
  "decayed:hot_to_warm": "Inactive for more than 1 day",
  "decayed:warm_to_cold": "Inactive for more than 2 days",
};
...
function formatReason(raw: string | null) {
  ...
  if (raw.startsWith("tanya_info:")) {
    ...
    return `Asked ${count}x about products`;
  }
...
function LeadCard({
  ...
  const name = lead.customer_name ?? "No name";
  const platform =
  ...
  return (
    ...
    >
      ...
      <div className="rounded-md bg-slate-50 px-3 py-2.5">
        <p className="text-[11px] font-medium text-slate-400 mb-0.5">Signal</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-md bg-slate-50 px-3 py-2">
          <p className="text-[11px] text-slate-400 mb-0.5">Interactions</p>
        ...
        <div className="rounded-md bg-slate-50 px-3 py-2">
          <p className="text-[11px] text-slate-400 mb-0.5">Last active</p>
          <p className="text-sm font-semibold text-slate-900 leading-snug">
            {relativeTime(lead.last_interaction)}
          </p>
          <p className="text-[11px] text-slate-400">
            joined {relativeTime(lead.created_at)}
          </p>
        </div>
      </div>
      ...
      {showActions && (
        <div className="flex gap-2 pt-1 border-t border-slate-100">
          <button
            ...
            aria-label={`Mark ${name} as resolved`}
          >
            Mark Resolved
          </button>
          <button
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6">
          ...
          <div className="mb-6 flex flex-col sm:flex-row gap-3">
            {hotCount > 0 && (
              <div className="flex items-center gap-2.5 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-red-700">{hotCount} hot prospects</p>
                  <p className="text-xs text-red-500">Needs immediate follow-up</p>
                </div>
              </div>
            )}
            {warmCount > 0 && (
              <div className="flex items-center gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-amber-700">{warmCount} warm prospects</p>
                  <p className="text-xs text-amber-600">Interested, keep monitoring</p>
                </div>
              </div>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 py-20 text-sm text-slate-400">
            ...
            <span>No leads in this category yet.</span>
          </div>
        ) : (
          <div
            className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3"
